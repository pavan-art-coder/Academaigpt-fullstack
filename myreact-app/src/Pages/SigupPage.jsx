...
import { PersonOutline, MailOutline, LockOutline, AutoStories } from '@mui/icons-material';
import { motion } from 'framer-motion';

// --- ANIMATION & THEME SETUP ---
...
const SignupPage = () => {
  ...
  return (
    <ThemeProvider theme={modernTheme}>
      <CssBaseline />
      <AnimatedBackground>
        <Box sx={{ zIndex: 1, width: '100%', maxWidth: 450, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          ...
          <motion.div style={{ width: '100%' }} variants={containerVariants} initial="hidden" animate="visible">
            <motion.div variants={itemVariants}>
              <Typography component="h1" variant="h4" align="center" gutterBottom> Create Your Account </Typography>
            </motion.div>
            <Box component="form" noValidate onSubmit={handleSignup} sx={{ mt: 1 }}>
              <motion.div variants={itemVariants}>
                <TextField variant="filled" margin="normal" required fullWidth label="Full Name" value={name} onChange={(e) => setName(e.target.value)} InputProps={{ startAdornment: (<InputAdornment position="start"><PersonOutline /></InputAdornment>), disableUnderline: true }} sx={{ backgroundColor: 'rgba(255, 255, 255, 0.08)', borderRadius: 2 }} />
              </motion.div>
              <motion.div variants={itemVariants}>
